...
def deduce_cash(df):
    ...
    # retailer cash 
    df["IS_RETAILER"] = df["MAKES"].apply(lambda x: [1 if f == "RETAILER" else 0 for f in x])
    df["IS_NOT_RETAILER"] = df["MAKES"].apply(lambda x: [1 if f != "RETAILER" else 0 for f in x])
    df["TOTAL_RETAILER_CASH"] = df[["IS_RETAILER", "CASH"]].apply(lambda x: np.nansum(np.array(x[0])*np.array(x[1])), axis=1)

    df["CASH_MEAN"] = df[["IS_NOT_RETAILER", "CASH"]].apply(lambda x: np.nanmean(np.array(x[0])*np.array(x[1])), axis=1)
    df["CASH_STD"] = df[["IS_NOT_RETAILER", "CASH"]].apply(lambda x: np.nanstd(np.array(x[0])*np.array(x[1])), axis=1)
    df["CASH_MIN"] = df[["IS_NOT_RETAILER", "CASH"]].apply(lambda x: np.nanmin(np.array(x[0])*np.array(x[1])), axis=1)
    
    df["CASH_MAX"] = df["CASH"].apply(lambda x : np.nanmax(x))
    df["CASH_TOTAL"] = df["CASH"].apply(lambda x : np.nansum(x))

    df["CASH_TOTAL_ITEMS"] = df["CASH_TOTAL"] - df["TOTAL_RETAILER_CASH"]

    # fulfilment cash 
    df["GOODSCODE_LIST"] = concatenate_columns(df, cols="GOODS_CODE")
    df["IS_FULFILMENT"] = df["GOODSCODE_LIST"].apply(lambda x: [1 if f == "FULFILMENT" else 0 for f in x])
    df["TOTAL_FULFILMENT_CASH"] = df[["IS_FULFILMENT", "CASH"]].apply(lambda x: np.nansum(np.array(x[0])*np.array(x[1])), axis=1)
